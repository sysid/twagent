        ...
        for agent_id in scope.agents:
            ...
            for capability in agent.capabilities:
                if only is not None and capability not in only:
                    continue
                _apply_one(
                    config, scope, agent, capability, expanded, result,
                    dry_run=dry_run, show_secrets=show_secrets,
                )
    ...
    *,
    ...
    if capability == "instructions":
        ...
        _deploy_file_artifacts(
            config, scope, agent, capability, expanded, paths, result,
            dry_run=dry_run,
        )
    elif capability == "mcp":
        _deploy_mcp(
            config, scope, agent, expanded, paths, result,
            dry_run=dry_run, show_secrets=show_secrets,
        )


def _resolve_target_paths(
    scope: Scope, agent: Agent, capability: str
) -> list[Path]:
    ...
    *,
    ...
    try:
        ...
        result.errors.append(
            f"render {scope.name}/{agent.id}/instructions: {exc}"
        )
        return
