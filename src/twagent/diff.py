    ...
    for scope in config.scopes:
        ...
        for agent_id in scope.agents:
            ...
            for cap in agent.capabilities:
                _diff_one(
                    config, scope, agent, cap, expanded, report,
...
def _diff_one(
    ...
    *,
    ...
    if cap == "instructions":
        ...
        _diff_mcp(config, scope, agent, expanded, targets, report, show_secrets=show_secrets)
    ...
    for target_dir in targets:
        for name, src in intended.items():
            ...
            if not link.exists() and not link.is_symlink():
                ...
                report.in_sync = False
                report.lines.append(
                    f"! {label}: real entry at {link} blocks deploy"
                )
            elif link.resolve() != src.resolve():
                report.in_sync = False
                report.lines.append(
                    f"~ {label}: was → {link.resolve()}; now → {src}"
                )
...
def _diff_mcp(config, scope, agent, expanded, targets, report, *, show_secrets):
    ...
    for target in targets:
        ...
        compare_current = _mask_json_text(current, intended_dict) if not show_secrets else current
